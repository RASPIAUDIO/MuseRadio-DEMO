...
{
    if (!s_uac_device || !next_accum || s_uac_device->spk_bytes_per_frame == 0 || interval_ms == 0) {
        return 0;
    }

    uint64_t frames_q = (uint64_t)s_uac_device->spk_frame_accum +
...
bool tud_audio_set_itf_cb(uint8_t rhport, tusb_control_request_t const *p_request)
{
    ...
    if (s_uac_device->spk_itf_num == itf && alt != 0) {
        ...
        s_uac_device->spk_bytes_per_frame = SPEAK_CHANNEL_NUM * s_uac_device->spk_resolution / 8;
        s_uac_device->spk_bytes_per_ms = (s_uac_device->current_sample_rate * s_uac_device->spk_bytes_per_frame) / 1000;
        s_uac_device->spk_frame_accum = 0;
        xTaskNotifyGive(s_uac_device->spk_task_handle);
        TU_LOG1("Speaker interface %d-%d opened", itf, alt);
        printf("Speaker interface %d-%d opened rate=%"PRIu32" frame_bytes=%u nominal_ms_bytes=%u\n",
               itf, alt, s_uac_device->current_sample_rate,
               (unsigned)s_uac_device->spk_bytes_per_frame,
               (unsigned)s_uac_device->spk_bytes_per_ms);
    }
#endif
...
bool tud_audio_rx_done_post_read_cb(uint8_t rhport, uint16_t n_bytes_received, uint8_t func_id, uint8_t ep_out, uint8_t cur_alt_setting)
{
     ...
     */
    if (last_time == 0 || now - last_time > (int64_t)CONFIG_UAC_SPK_NEW_PLAY_INTERVAL * 1000) {
        new_play = true;
        s_uac_device->spk_frame_accum = 0;
    ...
    uint32_t next_accum = s_uac_device->spk_frame_accum;
    size_t bytes_require = spk_bytes_for_ms(1, &next_accum);

    if (new_play) {
        /*!< Buffer a segment of data in the I2S and control the data size to be half of the UAC FIFO size. */
        const uint32_t prebuffer_ms = SPK_INTERVAL_MS > 1 ? (SPK_INTERVAL_MS / 2) : 1;
        bytes_require = spk_bytes_for_ms(prebuffer_ms, &next_accum);
