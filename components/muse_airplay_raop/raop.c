...
#define MUSE_AIRPLAY_TASK_CORE 1
...
#ifndef WIN32
static int raop_rng(void *ctx, unsigned char *output, size_t len)
{
	(void)ctx;
...
struct raop_ctx_s *raop_create(uint32_t host, char *name,
	...
	ctx->thread = xTaskCreateStaticPinnedToCore( (TaskFunction_t) rtsp_thread, "RTSP", RTSP_STACK_SIZE, ctx, 
												 ESP_TASK_PRIO_MIN + 2, ctx->xStack, ctx->xTaskBuffer, MUSE_AIRPLAY_TASK_CORE);
#endif
...
static bool handle_rtsp(raop_ctx_t *ctx, int sock)
{
	...
	if (!strcmp(method, "OPTIONS")) {
		...
		static const char info_plist[] =
			...
			"<key>initialVolume</key><real>-18.0</real>"
			"</dict></plist>\n";
		char content_length[12];
		response_body = info_plist;
		response_body_len = strlen(info_plist);
		snprintf(content_length, sizeof(content_length), "%u", (unsigned)response_body_len);
		kd_add(resp, "Content-Type", "text/x-apple-plist+xml");
		kd_add(resp, "Content-Length", content_length);
		...
		NFREE(ctx->rtsp.aesiv);
		NFREE(ctx->rtsp.fmtp);
		ctx->rtsp.raw_pcm = body && strcasestr(body, "L16/") != NULL;
		...
		rtp = rtp_init(ctx->peer, ctx->latency,	ctx->rtsp.aeskey, ctx->rtsp.aesiv,
					   ctx->rtsp.fmtp, ctx->rtsp.raw_pcm, cport, tport,
					   buffer, size, ctx->cmd_cb, ctx->data_cb);
	...
	if (success) {
		if (response_body) {
			buf = http_send_with_body(sock, "RTSP/1.0 200 OK", resp, response_body, response_body_len);
		} else {
...
static char *rsa_apply(unsigned char *input, int inlen, int *outlen, int mode)
{
	...
	const size_t rsa_len = mbedtls_rsa_get_len(trsa);

	switch (mode) {
	case RSA_MODE_AUTH:
		mbedtls_rsa_set_padding(trsa, MBEDTLS_RSA_PKCS_V15, MBEDTLS_MD_NONE);
		outbuf = malloc(rsa_len);
		if (outbuf && inlen <= (int)rsa_len - 11) {
			uint8_t *padded = malloc(rsa_len);
			if (padded) {
				padded[0] = 0x00;
				padded[1] = 0x01;
				memset(padded + 2, 0xff, rsa_len - (size_t)inlen - 3);
				padded[rsa_len - (size_t)inlen - 1] = 0x00;
				memcpy(padded + rsa_len - (size_t)inlen, input, inlen);
				rc = mbedtls_rsa_private(trsa, raop_rng, NULL, padded, outbuf);
				free(padded);
				if (rc == 0) *outlen = rsa_len;
			}
		}
		break;
	case RSA_MODE_KEY:
		...
		outbuf = malloc(rsa_len);
		if (outbuf) {
			rc = mbedtls_rsa_pkcs1_decrypt(trsa, raop_rng, NULL, &olen, input, outbuf, rsa_len);
			if (rc == 0) *outlen = olen;
		}
		break;
	}
