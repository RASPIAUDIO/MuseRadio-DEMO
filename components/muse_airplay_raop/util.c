...
/*----------------------------------------------------------------------------*/
char *http_send_with_body(int sock, char *method, key_data_t *rkd, const char *body, size_t body_len)
{
	unsigned sent, len;
	char *resp = kd_dump(rkd);
	char *data = malloc(strlen(method) + 2 + strlen(resp) + 2 + body_len + 1);

	len = sprintf(data, "%s\r\n%s\r\n", method, resp);
	NFREE(resp);

	if (body && body_len) {
		memcpy(data + len, body, body_len);
		len += body_len;
	}
	data[len] = '\0';
	...
	if (sent != len) {
		LOG_ERROR("HTTP send body error:%s %u (len=%u)", method, sent, len);
		NFREE(data);
	}
