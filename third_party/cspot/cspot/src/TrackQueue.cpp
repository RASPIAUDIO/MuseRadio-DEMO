...
namespace {
bool readProtoVarint(const std::vector<uint8_t>& data, size_t& pos,
                     uint64_t& value) {
  value = 0;
  uint8_t shift = 0;
...
bool readProtoLength(const std::vector<uint8_t>& data, size_t& pos,
                     size_t& length) {
  uint64_t value = 0;
  if (!readProtoVarint(data, pos, value) || value > data.size() - pos) {
    return false;
  }
...
bool skipProtoField(const std::vector<uint8_t>& data, size_t& pos,
                    uint32_t wireType) {
  switch (wireType) {
    case 0: {
      uint64_t ignored = 0;
      return readProtoVarint(data, pos, ignored);
    }
    case 1:
    ...
    case 2: {
      size_t length = 0;
      if (!readProtoLength(data, pos, length)) return false;
      pos += length;
      return true;
    }
    case 5:
...
std::string extractStorageResolveCdnUrl(const std::vector<uint8_t>& data) {
  size_t pos = 0;
  while (pos < data.size()) {
    uint64_t tag = 0;
    if (!readProtoVarint(data, pos, tag)) {
      break;
    }

    const uint32_t fieldNumber = (uint32_t)(tag >> 3);
    const uint32_t wireType = (uint32_t)(tag & 0x07);
    if (fieldNumber == 2 && wireType == 2) {
      size_t length = 0;
      if (!readProtoLength(data, pos, length)) {
        return "";
      }
      return std::string((const char*)data.data() + pos, length);
    }

    if (!skipProtoField(data, pos, wireType)) {
      break;
    }
  }
...
std::string bytesPreview(const std::vector<uint8_t>& data) {
  constexpr size_t MAX_PREVIEW = 24;
  std::string out;
  char byteHex[4];
  const size_t count = std::min(data.size(), MAX_PREVIEW);
...
void QueuedTrack::stepLoadCDNUrl(const std::string& accessKey) {
  ...
  try {
    ...
    static std::string spclientAddress;
    if (spclientAddress.empty()) {
      spclientAddress = ApResolve("").fetchFirstSpclientAddress();
      CSPOT_LOG(info, "Resolved spclient endpoint: %s",
                spclientAddress.c_str());
    }

    std::string country = ctx->config.countryCode.empty()
                              ? std::string("FR")
                              : ctx->config.countryCode;
    std::string requestUrl = string_format(
        "https://%s/storage-resolve/files/audio/interactive/%s"
        "?product=0&country=%s&salt=%llu",
        spclientAddress.c_str(), bytesToHexString(fileId).c_str(),
        country.c_str(), getCurrentTimestamp());
    ...
    // Wait for response
    auto resultBytes = req->bytes();
    cdnUrl = extractStorageResolveCdnUrl(resultBytes);
    if (cdnUrl.empty()) {
      std::string resultCopy(resultBytes.begin(), resultBytes.end());
      CSPOT_LOG(error,
                "Invalid CDN URL response status=%d retry-after=%s len=%d "
                "bytes=%s text=%.120s",
                req->status(), std::string(req->header("retry-after")).c_str(),
                (int)resultBytes.size(), bytesPreview(resultBytes).c_str(),
                resultCopy.c_str());
      throw std::runtime_error("Invalid CDN URL response");
    }
