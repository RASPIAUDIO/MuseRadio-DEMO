...
std::vector<uint8_t> LoginBlob::decodeBlob(
  ...
  if (blob.size() < 36) {
    throw std::runtime_error("Zeroconf blob is too small");
  }
  ...
  if (mac != checksum) {
    ...
    throw std::runtime_error("Zeroconf blob MAC mismatch");
  }
...
uint32_t LoginBlob::readBlobInt(const std::vector<uint8_t>& data) {
  if (blobSkipPosition >= data.size()) {
    throw std::runtime_error("Zeroconf blob parse overflow");
  }
  ...
  if (blobSkipPosition + 1 >= data.size()) {
    throw std::runtime_error("Zeroconf blob parse overflow");
  }
...
std::vector<uint8_t> LoginBlob::decodeBlobSecondary(
  ...
  if (l < 16) {
    throw std::runtime_error("Zeroconf blob secondary decode is too small");
  }
...
void LoginBlob::loadZeroconf(const std::vector<uint8_t>& blob,
  ...
  if (blobSkipPosition >= loginData.size()) {
    throw std::runtime_error("Zeroconf blob parse overflow");
  }
  blobSkipPosition += 1;
  ...
  if (blobSkipPosition + authSize > loginData.size()) {
    throw std::runtime_error("Zeroconf blob auth data is truncated");
  }
