...
void HTTPClient::Response::readChunkedBody() {
  ...
  while (true) {
    ...
    if (!std::getline(socketStream, chunkHeader)) {
      throw std::runtime_error("Cannot read chunk header");
    }

    if (!chunkHeader.empty() && chunkHeader.back() == '\r') {
      chunkHeader.pop_back();
    }

    const size_t extensionPos = chunkHeader.find(';');
    if (extensionPos != std::string::npos) {
      chunkHeader.resize(extensionPos);
    }

    const size_t chunkSize = std::stoul(chunkHeader, nullptr, 16);
    if (chunkSize == 0) {
      std::string trailer;
      while (std::getline(socketStream, trailer)) {
    ...
    const size_t offset = rawBody.size();
    rawBody.resize(offset + chunkSize);
    socketStream.read((char*)rawBody.data() + offset, chunkSize);
    if ((size_t)socketStream.gcount() != chunkSize) {
      throw std::runtime_error("Cannot read chunk body");
    }

    char crlf[2];
    socketStream.read(crlf, sizeof(crlf));
    if (socketStream.gcount() != sizeof(crlf)) {
      throw std::runtime_error("Cannot read chunk delimiter");
    }
  }
}
